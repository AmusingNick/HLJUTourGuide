...
public class PaintLinePause extends JFrame implements Runnable{
	//Point[0]为起始点,Point[1]为终止点
	public ArrayList<Point> ps = new ArrayList<Point>();
	public Graphics g;
	public int arcWeight=0;
	public PaintLinePause(){}
	public PaintLinePause(Point p1,Point p2,int arcWeight,Graphics g){
		ps.add(p1);
		ps.add(p2);
		this.arcWeight = arcWeight;
		this.g = g;
	}
	public PaintLinePause(Point p1,Point p2,Graphics g){
		ps.add(p1);
		ps.add(p2);
		this.g = g;
	}
	public PaintLinePause(MouseByImage mbi){
		ps.add(mbi.getpOne());
		ps.add(mbi.getpTwo());
	}
